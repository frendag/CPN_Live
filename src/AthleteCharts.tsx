...
// ─── Graphique 1 : Évolution des points ──────────────────────────────────────
...
function TempsChart({ ep }: { ep: EvoTempsEntry }) {
  ...
  return (
    ...
    >
      ...
      />
      <VictoryAxis dependentAxis tickFormat={(v: number) => secToStr(v)} style={axisStyle} />
      <VictoryLine
        data={realData}
...
// ─── Graphique 3 : Fréquence d'activité ──────────────────────────────────────
...
export default function AthleteCharts({ evo, evo_temps, freq }: Props) {
  ...
  return (
    <View style={styles.wrap}>
      ...
      {hasEvo && (
        ...
        >
          ...
          <View style={styles.statsRow}>
            <StatBox label="MEILLEUR" value={`${Math.max(...evo!.map(d => d.moy))} pts`} color={COLORS.accent} />
            <StatBox label="CETTE ANNÉE" value={`${evo![evo!.length - 1]?.moy} pts`} color={COLORS.warning} />
            <StatBox
              label="PROGRESSION"
              value={(() => { const last = evo![evo!.length - 1]; if (!last?.delta) return '—'; return `${last.delta > 0 ? '+' : ''}${last.delta}`; })()}
              color={(() => { const d = evo![evo!.length - 1]?.delta ?? 0; return d > 0 ? COLORS.success : d < 0 ? COLORS.danger : COLORS.muted; })()}
            />
          </View>
        </ChartSection>
      )}
      ...
      {hasTemps && (
        <ChartSection title="Suivi des temps" sub="Meilleure performance par trimestre">
          <View style={styles.bassinRow}>
            {epKeys50.length > 0 && (
              <Pressable style={[styles.bassinBtn, bassin === '50m' && styles.bassinBtnOn]} onPress={() => { setBassin('50m'); setSelectedEp(''); }}>
                <Text style={[styles.bassinBtnText, bassin === '50m' && styles.bassinBtnTextOn]}>50m</Text>
              </Pressable>
            )}
            {epKeys25.length > 0 && (
              <Pressable style={[styles.bassinBtn, bassin === '25m' && styles.bassinBtnOn]} onPress={() => { setBassin('25m'); setSelectedEp(''); }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.epRow}>
            {epKeys.map(k => {
              ...
              return (
                <Pressable key={k} onPress={() => setSelectedEp(k)}
                  style={[styles.epTab, { borderColor: active ? col : COLORS.border, backgroundColor: active ? col + '22' : COLORS.surface }]}
                >
                  <Text style={[styles.epTabText, { color: active ? col : COLORS.muted }]}>{k.replace(` · ${bassin}`, '')}</Text>
                </Pressable>
              );
            })}
          </ScrollView>
          ...
          {currentEp ? (
            <>
              ...
              <View style={styles.statsRow}>
                <StatBox label="MEILLEUR" value={currentEp.temps[currentEp.sec.indexOf(Math.min(...currentEp.sec))]} color={COLORS.success} />
                <StatBox label="DERNIER" value={currentEp.temps[currentEp.temps.length - 1]} />
                <StatBox label="MAX PTS" value={Math.max(...currentEp.pts)} color={COLORS.accent} />
                {currentEp.proj.length > 0 && (
                  <StatBox label="OBJECTIF" value={secToStr(currentEp.proj[0])} color={currentEp.color || COLORS.warning} />
                )}
              </View>
              {(() => {
                ...
                return (
                  <View style={styles.bestCompBox}>
                    ...
                    <Text style={styles.bestCompVal}>{comp}{lieu ? ` · ${lieu}` : ''}</Text>
                  </View>
                );
              })()}
              <View style={styles.legendRow}>
                <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: '#ff6b6b' }]} /><Text style={styles.legendText}>[INT]</Text></View>
                <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: '#ffa748' }]} /><Text style={styles.legendText}>[NAT]</Text></View>
                <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: currentEp.color || COLORS.primaryLight }]} /><Text style={styles.legendText}>Autre</Text></View>
              </View>
            </>
          ) : null}
        </ChartSection>
      ...
      {hasFreq && (
        <ChartSection title="Fréquence d'activité" sub="Nombre de performances par année" defaultOpen={false}>
          <FreqChart freq={freq!} />
          <View style={styles.statsRow}>
...
const styles = StyleSheet.create({
  ...
  sectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 14, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  ...
  bassinBtn:     { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 8, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.surface },
  ...
  statBox:       { flex: 1, minWidth: 70, backgroundColor: COLORS.surface, borderRadius: 10, borderWidth: 1, borderColor: COLORS.border, padding: 9, alignItems: 'center', gap: 3 },
  statLabel:     { color: COLORS.subtle, fontSize: 9, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.6 },
  statValue:     { color: COLORS.text, fontSize: 14, fontWeight: '900' },
  bestCompBox:   { backgroundColor: COLORS.surface, borderRadius: 8, borderWidth: 1, borderColor: COLORS.border, padding: 9, gap: 3 },
